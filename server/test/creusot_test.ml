...
let _test_hacky_rs_parser () =
  ...
  List.iter (fun (qname, _attrs, (start, stop)) ->
      Printf.eprintf "%s: %d:%d-%d:%d\n" (Rust_syntax.string_of_def_path qname) start.Lexing.pos_lnum (start.Lexing.pos_cnum - start.Lexing.pos_bol) stop.Lexing.pos_lnum (stop.Lexing.pos_cnum - stop.Lexing.pos_bol)) names
