let _test_hacky_rs_parser () =
  ...
  List.iter (fun (qname, (start, stop)) ->
    Printf.eprintf "%s: %d:%d-%d:%d\n" (Creusot_lsp.Hacky_rs_parser.string_of_qname qname) start.Lexing.pos_lnum (start.Lexing.pos_cnum - start.Lexing.pos_bol) stop.Lexing.pos_lnum (stop.Lexing.pos_cnum - stop.Lexing.pos_bol)) names
