...
function getCreusotVersion(): Array<number>|null {
  const raw_output = cargoCreusot(["creusot", "version"]);
  if (raw_output.status != 0) {
    window.showErrorMessage("Could not get cargo creusot version: failed to run `cargo creusot version`");
    return null;
  }
  const output = typeof raw_output.stdout == 'string' ? raw_output.stdout : raw_output.stdout.toString();
  const match = output.match(/cargo-creusot (\d+)\.(\d+)\.(\d+)(-[a-z]*)?/);
  if (match === null) {
    window.showErrorMessage("Could not get cargo creusot version: failed to parse output of `cargo creusot version`");
    return null;
  }
  return [+match[1], +match[2], +match[3]];
}

const version = getCreusotVersion();

function creusotVersionAtLeast(major: number, minor: number, fix: number): boolean {
  if (version === null) {
    // Either Creusot could not be found, or Creusot changed its version output; we can't do anything about the former so we assume the latter here.
    return true
  }
  return major < version[0] || major == version[0] && (minor < version[1] || minor == version[1] && (fix < version[2] || fix == version[2]))
}
